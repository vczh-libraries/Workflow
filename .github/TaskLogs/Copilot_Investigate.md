...
- Building GacUI after releasing Vlpp and VlppOS to GacUI.
...
Move the ChatBot dispatcher `TaskQueue` helper into VlppOS under `vl::threading`, directly after `RepeatingTaskExecutor`, then consume it from Workflow through the released VlppOS import. Update Workflow knowledge base text for `Console::TryRead` and `TaskQueue`, and update GacUI 1.4.0.0 new-feature notes.
