...
TODO_Task.md has been done on Windows, you are going to continue the unfinished verification work on Linux. Fix any issue you found, and limit the work on Workflow of course. commit and push once finished.
...
- Both projects are present in `UnitTest.sln`, `Project.md` documents their roles and invocation, and `Test/Linux/RpcStdioTest_Driver/vmake` plus `Test/Linux/RpcStdioTest_Service/vmake` describe portable builds that complete cleanly and interoperate on Linux.
...
- No.3 Use a portable completion event for bounded test-thread waits [CONFIRMED]
- No.4 Normalize nested project source entries for Linux makefile generation [CONFIRMED]
...
Linux verification produced the same compatibility boundary. Each of the 126 cases was isolated through the generated driver and service, yielding 70 successful cases and the same 56 shared-global failures as Windows. `StartRpcStdio.sh` then ran all 70 transport-compatible cases in fresh service processes with the exact 56-name skip file and exited with code 0.

## No.3 Use a portable completion event for bounded test-thread waits

The clean Linux `LibraryTest` build fails because four RPC JSON injection tests call `Thread::WaitForTime`, an API that is only exposed by the Windows `WaitableObject` implementation. Preserve the tests' five-second deadlock guard on every platform by giving their two stack-owned worker thread classes a shared completion event. The worker signals completion after either returning normally or catching the expected injected exception, and the test waits on that cross-platform event before joining the thread.

### CODE CHANGE

Add a small `CompletionThread` base in `TestLibraryRpcJson.cpp` that owns a manually-reset `EventObject`, exposes its portable timed wait, and lets `RequestThread` and `InitializeThread` signal completion at the end of their expected execution paths. Keep unexpected exceptions fail-fast and keep the existing unconditional thread join after the bounded completion check.

### CONFIRMED

The regenerated Linux `LibraryTest` project compiled the RPC JSON test source and its mock dependency successfully. `LibraryTest` then passed 3/3 files and 21/21 cases, including all four bounded injected-exception thread tests.

## No.4 Normalize nested project source entries for Linux makefile generation

The clean Linux stdio service build reports an invalid `realpath` and omits `TestCasesRpcStdio_Service.cpp`: the repository's project-to-vmake reader treats the complete line containing `ClCompile Include` as the path, while both new projects place the nested `/bigobj` `AdditionalOptions` element on that same line. Format these two nested `ClCompile` entries like the established `CppTest` projects, with the include element, nested option, and closing element on separate lines. This preserves identical MSBuild behavior while allowing the portable project reader to extract the source path.

### CODE CHANGE

Expanded the generated driver and service `ClCompile` entries to the established multi-line MSBuild form. Regenerating through `.github/Ubuntu/build.sh` now includes `TestCasesRpcStdio_Driver.cpp` and `TestCasesRpcStdio_Service.cpp` in each portable source list and link command.

### CONFIRMED

Both stdio projects completed clean Linux builds and linked their generated dispatchers. The complete required Linux sequence also passed: `LibraryTest` 21/21, `CompilerTest_GenerateMetadata` 2/2, both `CompilerTest_LoadAndCompile` passes 707/707, `RuntimeTest` 261/261, and each of `CppTest`, `CppTest_Metaonly`, and `CppTest_Reflection` 229/229. The prepared TypeScript package passed `npm run build` with `tsc --noEmit`.
