...
- Task 3 added `TODO_RPC_GeneratedWrappers.md`.
- The new document describes when generated C++ RPC functions are wired into a lifecycle and what each category does.
- It also separates JSON serialization entry points and explains how normal and JSON generated functions complete their work.
...
Task 3 is complete. The generated-wrapper document now covers lifecycle setup, generated object ops, event ops, caller-side ops, wrapper factory usage, service registration, JSON serializer setup, JSON ops selection, and completion behavior for calls, events, object lifetime, and serialization.
