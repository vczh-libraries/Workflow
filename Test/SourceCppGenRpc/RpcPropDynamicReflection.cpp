/***********************************************************************
!!!!!! DO NOT MODIFY !!!!!!

Source: ../Resources/Rpc/PropDynamic.txt; ../Resources/Rpc/PropDynamic_Client.txt; ../Resources/Rpc/PropDynamic_Service.txt

This file is generated by Workflow compiler
https://github.com/vczh-libraries
***********************************************************************/

#include "RpcPropDynamicReflection.h"

#if defined( _MSC_VER)
#pragma warning(push)
#pragma warning(disable:4250)
#elif defined(__clang__)
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wparentheses-equality"
#elif defined(__GNUC__)
#pragma GCC diagnostic push
#endif

/***********************************************************************
Reflection
***********************************************************************/

namespace vl
{
	namespace reflection
	{
		namespace description
		{
#ifndef VCZH_DEBUG_NO_REFLECTION
			IMPL_CPP_TYPE_INFO(RpcPropDynamic::IRpcWrapper_IService)
			IMPL_CPP_TYPE_INFO(RpcPropDynamic::IService)
			IMPL_CPP_TYPE_INFO(rpcops_IOps_Rpc_PropDynamic)

#ifdef VCZH_DESCRIPTABLEOBJECT_WITH_METADATA
#define _ ,
			BEGIN_INTERFACE_MEMBER(::RpcPropDynamic::IRpcWrapper_IService)
				CLASS_MEMBER_BASE(::RpcPropDynamic::IService)
				CLASS_MEMBER_BASE(::vl::rpc_controller::IRpcWrapperBase)
			END_INTERFACE_MEMBER(::RpcPropDynamic::IRpcWrapper_IService)

			BEGIN_INTERFACE_MEMBER(::RpcPropDynamic::IService)
				ATTRIBUTE_TYPE(::vl::__vwsn::att_rpc_Interface)
				ATTRIBUTE_TYPE(::vl::__vwsn::att_rpc_Ctor)
				CLASS_MEMBER_BASE(::vl::reflection::IDescriptable)
				CLASS_MEMBER_METHOD(GetValue, NO_PARAMETER)
				CLASS_MEMBER_METHOD(SetValue, { L"value" })
				CLASS_MEMBER_METHOD(Signal, NO_PARAMETER)
				CLASS_MEMBER_EVENT(ValueChanged)
				CLASS_MEMBER_PROPERTY_EVENT_READONLY(Value, GetValue, ValueChanged)
				ATTRIBUTE_MEMBER(::vl::__vwsn::att_rpc_Dynamic)
			END_INTERFACE_MEMBER(::RpcPropDynamic::IService)

			BEGIN_INTERFACE_MEMBER(::rpcops_IOps_Rpc_PropDynamic)
				CLASS_MEMBER_BASE(::vl::reflection::IDescriptable)
				CLASS_MEMBER_METHOD(InvokeEvent_RpcPropDynamic__IService_ValueChanged, { L"ref" })
				CLASS_MEMBER_METHOD(InvokeMethod_RpcPropDynamic__IService_GetValue, { L"ref" })
				CLASS_MEMBER_METHOD(InvokeMethod_RpcPropDynamic__IService_SetValue, { L"ref" _ L"arg_value" })
				CLASS_MEMBER_METHOD(InvokeMethod_RpcPropDynamic__IService_Signal, { L"ref" })
			END_INTERFACE_MEMBER(::rpcops_IOps_Rpc_PropDynamic)

#undef _
			class Rpc_PropDynamicTypeLoader : public Object, public ITypeLoader
			{
			public:
				void Load(ITypeManager* manager)
				{
					ADD_TYPE_INFO(::RpcPropDynamic::IRpcWrapper_IService)
					ADD_TYPE_INFO(::RpcPropDynamic::IService)
					ADD_TYPE_INFO(::rpcops_IOps_Rpc_PropDynamic)
				}

				void Unload(ITypeManager* manager)
				{
				}
			};
#endif
#endif

			bool LoadRpc_PropDynamicTypes()
			{
#ifdef VCZH_DESCRIPTABLEOBJECT_WITH_METADATA
				if (auto manager = GetGlobalTypeManager())
				{
					return manager->AddTypeLoader(Ptr(new Rpc_PropDynamicTypeLoader));
				}
#endif
				return false;
			}
		}
	}
}

#if defined( _MSC_VER)
#pragma warning(pop)
#elif defined(__clang__)
#pragma clang diagnostic pop
#elif defined(__GNUC__)
#pragma GCC diagnostic pop
#endif
