...
namespace RpcWrapperValidation
{
	...
	interface IService
	{
		...
		TEST_CASE(L"TestCasesRpc.cpp")
		{
			...
			writer.WriteLine(L"\t\t{");
			writer.WriteLine(L"\t\t\tDisconnectTrackedWrappers();");
			writer.WriteLine(L"\t\t}");
			...
			writer.WriteLine(L"\tauto lo2 = Ptr(new RpcCalleeListOps(lc2.Obj()));");
			...
			writer.WriteLine(L"\tlc1->RegisterWrapperFactory([&](vint typeId, IRpcLifeCycle* lc) { (void)lc; return instance.rpcwrapper_Create(typeId, lc1.Obj()); });");
			writer.WriteLine(L"\tlc2->RegisterWrapperFactory([&](vint typeId, IRpcLifeCycle* lc) { (void)lc; return instance.rpcwrapper_Create(typeId, lc2.Obj()); });");
			writer.WriteLine(L"");
			writer.WriteLine(L"\tinstance.serviceMain(lc1.Obj());");
			writer.WriteLine(L"");
			writer.WriteLine(L"\tauto actual = instance.clientMain(lc2.Obj());");
			writer.WriteLine(L"");
			...
			writer.WriteLine(L"");
			writer.WriteLine(L"\tlc2->DisconnectTrackedWrappersBeforeDispose();");
			writer.WriteLine(L"\tlc1->DisconnectTrackedWrappersBeforeDispose();");
			writer.WriteLine(L"}");
