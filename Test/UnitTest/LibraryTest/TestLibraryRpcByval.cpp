...
namespace
{
	...
	RpcTestContext CreateContext()
	{
		...
		context.lifecycle1->GetController()->Register(
			Ptr(new RpcCalleeObjectOpsForList(objectOps1, context.listOps1)),
			Ptr(new RpcCalleeObjectEventOpsForList(objectEventOps1, context.listEventOps1)),
			context.listOps1,
			context.listEventOps1);
		context.lifecycle2->GetController()->Register(
			Ptr(new RpcCalleeObjectOpsForList(objectOps2, context.listOps2)),
			Ptr(new RpcCalleeObjectEventOpsForList(objectEventOps2, context.listEventOps2)),
			context.listOps2,
			context.listEventOps2);
